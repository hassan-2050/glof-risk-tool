...
def stage07_watcher_eval(cfg: Config) -> dict:
    ...
    headline = {
        ...
        # Count derived, never typed. Chamoli holds no water, so it carries no
        # ratio and is not in `ranked`: the study set is 14 sites but only 13
        # are rankable, and an earlier hardcoded "fourteen" here contradicted
        # the "1 of 13" three lines above it.
        "threshold_free_statement": (
            f"Thame ranks first of {len(ranked)} on the continuous "
            "source-to-lake volume ratio computed from pre-event data alone; "
            "this statement does not depend on any alarm threshold. The "
            "fourteenth site, Chamoli, impounds no water and so has no ratio "
            "to rank."),
    }
...
def stage17_packaging(cfg: Config) -> dict:
    ...
    # README drift gate.
    #
    # The note at the bottom of this record used to claim a figure "cannot
    # drift from the run" because Stage 18 writes the documentation. That was
    # only ever true of docs/RESULTS.md, LIMITS.md and ETHICS.md. README.md is
    # hand-written, nothing checked it, and it did drift: it carried recall
    # 0.333/0.667 and Spearman 0.63 - all three superseded by the delineation
    # correction (DECISIONS D13) - while claiming on the same page that "every
    # figure below is generated by the run". A project whose argument is that
    # unverified numbers are the hazard cannot leave its own front page
    # unverified, so the claim is now enforced rather than asserted.
    # Values are compared numerically, not as strings: the README rounds for
    # display (0.950 for 0.95, 0.096 for 0.0958) and a substring test would
    # flag correct prose as stale. Agreement to three decimals is the bar.
    import re as _re

    readme_text = (REPO_ROOT / "README.md").read_text(encoding="utf-8")

    def _readme_number(pattern: str):
        m = _re.search(pattern, readme_text)
        return float(m.group(1)) if m else None

    _row = r"\|\s*{}\s*\|\s*\*{{0,2}}([0-9.]+)\*{{0,2}}\s*\|\s*\*{{0,2}}([0-9.]+)"

    def _row_pair(label: str):
        m = _re.search(_row.format(label), readme_text)
        return (float(m.group(1)), float(m.group(2))) if m else (None, None)

    _recall_base, _recall_adv = _row_pair("recall")
    _contra_base, _contra_adv = _row_pair("contradiction recall")
    _hall_base, _hall_adv = _row_pair("hallucination rate")
    readme_expect = {
        "watcher recall, growth-only":
            (_recall_base, headline["watcher_recall_growth_only"]),
        "watcher recall, proxy-augmented":
            (_recall_adv, headline["watcher_recall_proxy_augmented"]),
        "Spearman vs Rounce 2017":
            (_readme_number(r"expert classes: \*\*([0-9.]+)\*\*"),
             headline["spearman_vs_rounce_2017"]),
        "reporter contradiction recall, baseline":
            (_contra_base, headline["reporter_contradiction_recall_baseline"]),
        "reporter contradiction recall, advanced":
            (_contra_adv, headline["reporter_contradiction_recall_advanced"]),
        "reporter hallucination rate, baseline":
            (_hall_base, headline["reporter_hallucination_baseline"]),
        "reporter hallucination rate, advanced":
            (_hall_adv, headline["reporter_hallucination_advanced"]),
        "contradiction F1":
            (_readme_number(r"hand-labelled key: \*\*([0-9.]+)\*\*"),
             headline["contradiction_f1"]),
    }
    readme_stale = sorted(
        k for k, (found, want) in readme_expect.items()
        if found is None or abs(found - want) > 5e-4)
    # The rank is a string, not a rounded number, so it is matched verbatim.
    if f"**Thame ranks {headline['thame_proxy_rank']}**" not in readme_text:
        readme_stale.append("Thame rank")

    # The limits paragraph is hand-written too, and it is where the stalest
    # figures were found (Imja 0.07x and "every event-bracket scene for South
    # Lhonak and Pyurepu is cloud-obscured", both false by the time they were
    # read). Its two count claims are checked the same way.
    # The demo script gets the same treatment as the README, and for a sharper
    # reason: these are figures a person says out loud to an audience that
    # cannot check them. Drafting it introduced a wrong one immediately
    # (Chamoli's residual water as 700 m² when the run says 300), which is the
    # fifth typed figure in this repository to drift from the artefacts.
    # Presence, not position. An earlier version pinned each figure to the
    # sentence it sat in, and the first rewrite of the script broke three checks
    # that were not actually stale - a gate that cries wolf on a reword gets
    # switched off. This asks only that the run's own value still appears
    # somewhere in the document, which is what "the figures are current" means.
    _pre_val = [r["validation"] for r in
                read_json(outputs / "stage02_delineation.json")["lakes"]
                if r.get("validation")]
    demo_path = REPO_ROOT / "docs" / "DEMO-STORY.md"
    demo_stale: list[str] = []
    if demo_path.exists():
        demo = demo_path.read_text(encoding="utf-8")
        thame = weval["per_lake"]["thyanbo_tsho"]
        for label, needle in (
            ("Thame pre-event area", f"{thame['growth_only']['area_km2']} km²"),
            ("Thame rank", weval["headline"]["thame_proxy_rank_of_n"]),
            ("Spearman", str(weval["spearman_vs_rounce_2017"])),
            ("Chamoli residual water",
             f"{neg['watcher']['evidence']['water_found_m2']:,.0f} m²"),
            ("delineation validated",
             f"{sum(1 for r in _pre_val if r['within_25pct'])}/{len(_pre_val)}"),
        ):
            if needle not in demo:
                demo_stale.append(f"{label} (run says {needle!r})")

    _delin = read_json(outputs / "stage02_delineation.json")
    _v = [r["validation"] for r in _delin["lakes"] if r.get("validation")]
    _pre_scenes = [s for L in _delin["lakes"] for s in L["scenes"]
                   if s.get("role") == "event_pre"]
    _pre_ok = sum(1 for s in _pre_scenes if not s["qa"]["reasons"])
    for label, pattern, want in (
        ("delineation validated",
         r"validates on \*\*(\d+) of (\d+)\*\* lakes",
         (sum(1 for x in _v if x["within_25pct"]), len(_v))),
        ("usable pre-event scenes",
         r"\*\*only (\d+) of the (\d+) pre-event scenes\*\*",
         (_pre_ok, len(_pre_scenes))),
    ):
        m = _re.search(pattern, readme_text)
        if not m or (int(m.group(1)), int(m.group(2))) != want:
            readme_stale.append(label)
    ...
    problems += [f"README figure stale: {k}" for k in readme_stale]
    problems += [f"DEMO-STORY figure stale: {k}" for k in demo_stale]

    # outputs/<TOOL_OUTPUT_DIR>/ is excluded from the run manifest so that a
    # tool-built page cannot pollute the determinism comparison. That exclusion
    # is only safe while no reproduce stage writes there - otherwise a stage
    # could emit an unhashed artefact and the byte-identity guarantee would have
    # a hole in it. Checked rather than assumed.
    from src.common.io import TOOL_OUTPUT_DIR
    from src.common.stages import reproduce_stages
    leaked = sorted(
        o for st in reproduce_stages() for o in (st.outputs or ())
        if o.replace("\\", "/").startswith(f"outputs/{TOOL_OUTPUT_DIR}/"))
    problems += [f"stage output inside the unhashed tool directory: {o}"
                 for o in leaked]
    record = {
        ...
        "readme_headline_figures_current": not readme_stale,
        "readme_stale_figures": readme_stale,
        "demo_story_figures_current": not demo_stale,
        "demo_story_stale_figures": demo_stale,
        "offline_guard_engaged_during_run": offline_engaged(),
        "problems": problems,
        "note": ("Stage 18 writes docs/RESULTS.md, LIMITS.md and ETHICS.md FROM "
                 "headline_numbers. README.md is hand-written, so it is checked "
                 "against headline_numbers instead and this stage fails if it "
                 "has drifted. If a reviewer's numbers differ, compare "
    ...
    return {"headline_numbers": len(headline),
            ...
            "readme_current": not readme_stale,
...
{len(weval['ranking'])} values (see DECISIONS D7) and is therefore not a blind
...
"""
    ...
    # Derived, not typed. The three ratios that used to be hardcoded in the
    # calving bullet (Imja 0.07x, Tsho Rolpa 0.12x, South Lhonak 0.33x) were
    # measured BEFORE the SCL-extension fix and survived it unchanged, so a
    # generated document was quoting numbers the run had already superseded.
    import textwrap as _tw

    _val = [(r["name"], r["validation"]["ratio_to_published"])
            for r in delin["lakes"] if r.get("validation")]
    _off = sorted(((n, x) for n, x in _val if abs(x - 1.0) > 0.25),
                  key=lambda nx: nx[1])
    _fmt = lambda pairs: ", ".join(f"{n.split(' (')[0]} {x:.2f}x"
                                   for n, x in pairs)
    # Each direction is named only when it has members. An earlier version
    # printed "Over-measured: ." once D14 emptied that category, which reads
    # like a truncated sentence rather than the absence it actually is.
    _under = _fmt([(n, x) for n, x in _off if x < 1.0])
    _over = _fmt([(n, x) for n, x in _off if x > 1.0])
    _dirs = ". ".join(
        f"{lbl}: {v}" for lbl, v in (("Under-measured", _under),
                                     ("Over-measured", _over)) if v)
    _why = ("Under-measurement is floating ice and debris breaking the water "
            "into disconnected patches, which any largest-connected-component "
            "rule loses; over-measurement is the opposite failure, adjacent "
            "wet ground and shadow joining the lake.")
    _calving = _tw.fill(
        (f"* **Absolute areas are still unreliable on {len(_off)} of "
         f"{len(_val)} validated lakes**, against published references. "
         f"{_dirs}. {_why} Ruled out by measurement: thresholds, closing "
         f"radius, floating-ice inclusion, and ESA's own classifier "
         f"(DECISIONS D6). Absolute areas for these lakes must not feed an "
         f"area screen."
         if _off else
         f"* **Absolute areas now validate on all {len(_val)} lakes** that "
         f"carry a published reference within 25%, but two of those passes "
         f"came from correcting the reference rather than the measurement "
         f"(DECISIONS D14). The ratio is only ever as good as its "
         f"denominator."),
        width=78, subsequent_indent="  ")
    _holdout = _tw.fill(
        f"* **One threshold is not a blind holdout.** The source-to-lake volume "
        f"alarm level was chosen after inspecting all "
        f"{len(weval['ranking'])} values - all {len(weval['ranking'])}, not all "
        f"{len(weval['per_lake'])}, because Chamoli impounds no water and "
        f"carries no ratio. The threshold-free rank statement is the defensible "
        f"one and is what the headline uses.",
        width=78, subsequent_indent="  ")

    # Also derived. The hardcoded version claimed "every event-bracket scene for
    # South Lhonak and Pyurepu is cloud-obscured" - the run disagrees: South
    # Lhonak has two clean post-event scenes and Pyurepu one clean pre-event
    # scene. The real limit is narrower and sharper, so state the real one.
    _pre = [(L["name"], s) for L in delin["lakes"] for s in L["scenes"]
            if s.get("role") == "event_pre"]
    _pre_clean = [(k, s) for k, s in _pre if not s["qa"]["reasons"]]
    _n_events = len({k for k, _ in _pre})
    _blind = sorted({k for k, _ in _pre} - {k for k, _ in _pre_clean})
    _cloud = _tw.fill(
        f"* **Optical monitoring is blindest exactly when GLOFs happen.** Only "
        f"{len(_pre_clean)} of the {len(_pre)} pre-event scenes across our "
        f"{_n_events} events clears the cloud and snow QA gate. "
        f"{len(_blind)} of {_n_events} events - "
        f"{', '.join(n.split(' (')[0] for n in _blind)} - have no usable "
        f"pre-event scene at all, so the last measurement before the burst "
        f"comes from the annual series weeks or months earlier. Three of the "
        f"four events fall in or near monsoon season. This is a strong "
        f"argument for Sentinel-1 SAR fusion and a real limit on any "
        f"optical-only system.",
        width=78, subsequent_indent="  ")

    _exp = read_json(outputs / "stage05_exposure.json")
    _counts = {}
    for _L in _exp["lakes"]:
        for _k, _v in (_L.get("counts") or {}).items():
            _counts[_k] = _counts.get(_k, 0) + _v
    _exposure = _tw.fill(
        f"* **Exposure counts are lower bounds, and weak ones.** Corridors are "
        f"truncated by a 6 km analysis window while the Thame flood carried "
        f"debris 80 km and South Lhonak's inundation ran 169 km. "
        f"{len(_exp['lakes'])} lakes yield "
        f"{', '.join(f'{v} {k}' + ('s' if v != 1 else '') for k, v in sorted(_counts.items())) or 'no assets'} "
        f"and no population at all. Meaningful exposure needs a river-network "
        f"domain, not a lake-centred window (DECISIONS D11).",
        width=78, subsequent_indent="  ")
